/********************************************************
  ...
  -------------------------------------------------------
   文 件 名   : Func_TD300_Reagent_Arm.c
   版 本 号   : V1.0.0.0
   作    者   : ryc
   生成日期   : 2019年10月17日
   功能描述   : TD300试剂臂功能定义
...
#include "Func_TD300_Reagent_Arm.h"
...
double ReagentLeve[DSK_NUMBER_MAX] = {0};	/*记录试剂液面高度*/
double ReactionLeve[85] = {0};	/*记录反应杯液面高度*/
extern uint16_t DskNumberNow;		/*当前位号，即当前盘逻辑1号位置对应的物理位置*/
static uint8_t _gp_InjStatus = 0;	/*吸样标志位，0已排样，1已吸样*/
//static DRV_AT24C32_TYPE* _gp_EEP = NULL;    

static MCC_PARAMETER1_TYPE _para1_VRT = {0x11, 16, 1.8*10, 42*100, 250,
										  MCC_RESET_POSITIVE,MCC_SHELTER_YES,MCC_SHELTER_YES, MCC_SHELTER_NO,13,
										  100, 1200,  4,  40, 20,
										  100, 1200,  4,  40, 20};
static MCC_PARAMETER2_TYPE _para2_VRT[4] = {{0x11, DISENABLE_CLOSE_CYCLE, 0.0*100, -1.0*100, 1.0*100, 1.0*100, 1.0*100},
											{0x11, ENABLE_CLOSE_CYCLE, 112.32*100, 110.55*100, 114.55*100, 8.0*100, 1.0*100},
...
static MCC_PARAMETER1_TYPE _para1_RTT = {0x11, 16, 1.8*10, 72*100, 180,
										  ...
										  100,	1300,	16, 50, 30,
                                          100,	800,	16, 20,	15};
static MCC_PARAMETER2_TYPE _para2_RTT[5] = {{0x11, DISENABLE_CLOSE_CYCLE, -33.04*100, -30.44*100, -34.94*100, 1.0*100, 1.0*100},
											 {0x11,DISENABLE_CLOSE_CYCLE, -20.88*100, 18.52*100, -21.94*100, 1.0*100, 1.0*100},
											 {0x11,DISENABLE_CLOSE_CYCLE, 0.0*100, -1.0*100, 1.0*100, 1.0*100, 1.0*100},
											 {0x11,DISENABLE_CLOSE_CYCLE, 30.32*100, 29.60*100, 31.60*100, 1.0*100, 1.0*100},
											 {0x11,DISENABLE_CLOSE_CYCLE, 40.82*100, 38.60*100, 42.60*100,1.0*100,1.0*100}};

static MCC_PARAMETER1_TYPE _para1_INJ = {0x11, 16, 1.8*10, 100*100, 1000,
...
static REAGENT_NEEDLE_PARAMETER_TYPE _para_NEEDLE = {0x11, 15.0*100, 1.5*100, 15.0*100, 500, 500, 5.0*100, 4.5*100, };
...
bool Func_TD300ReagentArmInit(void)
{
	...
	MCC_SetPresetLocation(_gp_VRT, 1, _para2_VRT[1].en_close_cycle, _para2_VRT[1].data_multiply100/100.0, _para2_VRT[1].forward_pre_deceleration_data_multiply100/100.0, _para2_VRT[1].reverse_pre_deceleration_data_multiply100/100.0, _para2_VRT[1].forward_compensation_multiply100/100.0,_para2_VRT[1].reverse_compensation_multiply100/100.0);		/*预设位置2：吸样垂直凌空位置坐标，正向预减速坐标，反向预减速坐标，正向补偿，反向补偿*//*15.75, 14.75, 116.55, 1.0, 1.0*/
	MCC_SetPresetLocation(_gp_VRT, 2, _para2_VRT[2].en_close_cycle, _para2_VRT[2].data_multiply100/100.0, _para2_VRT[2].forward_pre_deceleration_data_multiply100/100.0, _para2_VRT[2].reverse_pre_deceleration_data_multiply100/100.0, _para2_VRT[2].forward_compensation_multiply100/100.0,_para2_VRT[2].reverse_compensation_multiply100/100.0);		/*预设位置3：排样垂直凌空位置坐标，正向预减速坐标，反向预减速坐标，正向补偿，反向补偿*//*16.8, 15.8, 48.72, 1.0, 1.0*//*16.8, 15.8*/
	MCC_SetPresetLocation(_gp_VRT, 3, _para2_VRT[3].en_close_cycle, _para2_VRT[3].data_multiply100/100.0, _para2_VRT[3].forward_pre_deceleration_data_multiply100/100.0, _para2_VRT[3].reverse_pre_deceleration_data_multiply100/100.0, _para2_VRT[3].forward_compensation_multiply100/100.0,_para2_VRT[3].reverse_compensation_multiply100/100.0);		/*预设位置4：洗针位置坐标，正向预减速坐标，反向预减速坐标，正向补偿，反向补偿*/
	MCC_SetStempMotorLine(_gp_VRT, _para1_VRT.start_fre_h, _para1_VRT.end_fre_h, _para1_VRT.ech_lader_step_h, _para1_VRT.up_max_lader_h, _para1_VRT.s_par_h_divide100*100, 0);	/*高速曲线*//*800,	1200,	1, 35,	10000, 0*//*总行程550步，瓶上部50，中部300*/		/*200, 1200, 4, 40, 2000, 0*/
	MCC_SetStempMotorLine(_gp_VRT, _para1_VRT.start_fre_l, _para1_VRT.end_fre_l, _para1_VRT.ech_lader_step_l, _para1_VRT.up_max_lader_l, _para1_VRT.s_par_l_divide100*100, 1);	/*低速曲线*//*800,	1200,	1, 35,	10000, 1*//*200,	1000,	16, 50,		2500, 0*/     /*200,	1200, 4, 40, 2000, 1*/
	...
	_gp_INJ = MCC_Init(MCC_3,
						_para1_INJ.subdivision, _para1_INJ.step_angle_multiply10/10.0, _para1_INJ.ratio_multiply100/100.0, _para1_INJ.max_step,	/*细分、步距角、电机转一圈的行程、最大行程*/
	...
	/*外壁清洗阀----------------------------------------------------*/
	_gp_OuterWallCleaningValve = Valve_Init(VALVE_3, 
											VALVE_NORMAL_CLOSE, VALVE_POSITIVE_LOGIC,	/*常闭阀、正逻辑控制*/
											_PA_, _P7_);
	if(_gp_InnerWallCleaningValve == NULL)	/*模块初始化失败*/
	{
	...
	while(1)
	{
		...
		if(_gp_InjStatus == 0)	/*已经排样*/
		{
			err = MCC_Move_Distance(_gp_INJ,
									!_gp_INJ->reset_dir,
									(double)_para_NEEDLE.up_inhaled_air_after_multiply100/100.0,				/*单位uL，吸15uL空气*/
									5000,
									MCC_MOVE_DISTANCE,
									0,
									MCC_LINE_AUTO);
		...
		else					/*已经吸样*/
		{
			err = MCC_Move_Distance(_gp_INJ,
									!_gp_INJ->reset_dir,
									(double)_para_NEEDLE.up_inhaled_air_befor_multiply100/100.0,				/*单位uL，吸1.5uL空气*/
									5000,
									MCC_MOVE_DISTANCE,
									0,
									MCC_LINE_AUTO);
...
void TaskResident2(void* p_arg)
{
	...
	while(1)
	{
		...
		err = MCC_Move_Distance(_gp_INJ,
								!_gp_INJ->reset_dir,
								(double)_para_NEEDLE.rotate_inhaled_air_multiply100/100.0,				/*单位uL，吸15uL空气*/
								5000,
								MCC_MOVE_DISTANCE,
								0,
								MCC_LINE_AUTO);
...
//										_gp_VRT->preset_location + 3,	/*按预设位置下降到清洗位*/
//										5000,
//										MCC_MOVE_DISTANCE,
//										0,
//										MCC_LINE_AUTO);
...
//		}
		...
		OSTimeDlyHMSM(0, 0, 0, 10);				/*延时等待阀关严*/
...
bool Func_CmdNdlUp(void* p_buffer)
{
	...
	switch(RttNumberNow)	/*上升要吸空气*/
	{
		case 0: /*内圈吸样位*/
		case 1:	/*外圈吸样位*/
		case 3:	/*R1排样位*/
		case 4: /*R2排样位*/
			OSSemPost(_gp_UpInhaleSemIn);	/*试剂针在吸样位上升同步吸空气1.5uL，在排样位上升同步吸空气15uL*/
		break;
	}
...
bool Func_CmdNdlDown(void* p_buffer)
{
	...
	{
		if((RttNumberNow == 0) || (RttNumberNow == 1))	/*针在吸样位置*/
		{
//			double temp = 0;
//			
//			if((RttNumberNow == 0) && (ReagentLeve[(4 + 36 - DskNumberNow) % 36] != 0))	/*内圈记录有数据*/
//			{
//				/*先写入下降高度*/
//				temp = (_gp_VRT->preset_location + 1)->forward_pre_deceleration_data;	/*记录预减速坐标*/
//				(_gp_VRT->preset_location + 1)->forward_pre_deceleration_data = ReagentLeve[(4 + 36 - DskNumberNow) % 36];	/*修改预减速坐标*/
//			}
//			else if((RttNumberNow == 1) && (ReagentLeve[(1 + 36 - DskNumberNow) % 36] != 0))	/*外圈记录有数据*/
//			{
//				/*先写入下降高度*/
//				temp = (_gp_VRT->preset_location + 1)->forward_pre_deceleration_data;	/*记录预减速坐标*/
//				(_gp_VRT->preset_location + 1)->forward_pre_deceleration_data = ReagentLeve[(1 + 36 - DskNumberNow) % 36];	/*修改预减速坐标*/
//			}
//			/*再运动*/
			err = MCC_Move_PresetLocation(_gp_VRT,
											_gp_VRT->preset_location + 1,	/*按预设吸样位置下降*/
											5000,
											MCC_MOVE_D_CODE,
...
//			if((RttNumberNow == 0) && (ReagentLeve[(4 + 36 - DskNumberNow) % 36] != 0))	/*内圈记录有数据*/
//			{
//				(_gp_VRT->preset_location + 1)->forward_pre_deceleration_data = temp;	/*恢复预减速坐标*/
//			}
//			else if((RttNumberNow == 1) && (ReagentLeve[(1 + 36 - DskNumberNow) % 36] != 0))	/*外圈记录有数据*/
//			{
//				(_gp_VRT->preset_location + 1)->forward_pre_deceleration_data = temp;	/*恢复预减速坐标*/
//			}
		...
		else if((RttNumberNow == 3) || (RttNumberNow == 4))	/*针在排样位置*/
		{
			err = MCC_Move_PresetLocation(_gp_VRT,
											_gp_VRT->preset_location + 2,	/*按预设排样位置下降*/
											5000,
											MCC_MOVE_D_CODE,
											MCC_GetDCodeInterval(_gp_VRT, MCC_CalcSubdivisionStep(_gp_VRT, (_gp_VRT->preset_location + 1)->data)),
											MCC_LINE_AUTO);
		...
		/*计算当前位置，整形，含两位小数*/
		plase = (int32_t)(((double)(_gp_VRT->position) / (double)(_gp_VRT->subdivision)) * ((_gp_VRT->step_angle) / (double)360) * (_gp_VRT->ratio) * 100);	
	
//		/*针在吸样位置，记录下降高度*/
//		if(RttNumberNow == 0)
//		{
//			ReagentLeve[(4 + 36 - DskNumberNow) % 36] = (double)plase / 100.0 - 3.0;
//			if(ReagentLeve[(4 + 36 - DskNumberNow) % 36] > 114)
//				ReagentLeve[(4 + 36 - DskNumberNow) % 36] = 114;
//			else if(ReagentLeve[(4 + 36 - DskNumberNow) % 36] < 10)
//				ReagentLeve[(4 + 36 - DskNumberNow) % 36] = 10;
//		}
//		else if(RttNumberNow == 1)
//		{
//			ReagentLeve[(1 + 36 - DskNumberNow) % 36] = (double)plase / 100.0 - 3.0;
//			if(ReagentLeve[(1 + 36 - DskNumberNow) % 36] > 114)
//				ReagentLeve[(1 + 36 - DskNumberNow) % 36] = 114;
//			else if(ReagentLeve[(1 + 36 - DskNumberNow) % 36] < 10)
//				ReagentLeve[(1 + 36 - DskNumberNow) % 36] = 10;
//		}
	}
	else	/*不进行液面检测*/
	{
		...
		else if((RttNumberNow == 3) || (RttNumberNow == 4))	/*针在排样位置，随量跟踪*/
		{
			...
			tempf = (_gp_VRT->preset_location + 2)->forward_pre_deceleration_data;	/*记录当前预减速坐标*/
			tempd = (_gp_VRT->preset_location + 2)->data;							/*记录当前目标坐标*/
			
			if((liquid_h > 2.5) && (liquid_h < (_gp_VRT->preset_location + 2)->forward_pre_deceleration_data))	/*如果液面高度大于2.5mm，需要探入液面2mm*/
			{
				(_gp_VRT->preset_location + 2)->forward_pre_deceleration_data -= (liquid_h - 4);	/*-4为下降补偿*/
				(_gp_VRT->preset_location + 2)->data -= (liquid_h - 4);
				...
				(_gp_VRT->preset_location + 2)->forward_pre_deceleration_data = tempf;
				(_gp_VRT->preset_location + 2)->data = tempd;
			}
			else if((liquid_h <= 2.5) && (liquid_h >= 0))	/*液量不够下探*/
			{
				(_gp_VRT->preset_location + 2)->forward_pre_deceleration_data -= liquid_h;
				(_gp_VRT->preset_location + 2)->data -= liquid_h;
	...
	if(p_data->en_reporting_margin == 1)	/*使能余量上报*/
	{
		...
		/*计算液面高度*/
		if((RttNumberNow == 0) || (RttNumberNow == 1))	/*针在吸样位置*/
			plase = (uint32_t)((_gp_VRT->preset_location + 1)->reverse_pre_deceleration_data * 100) - plase;
		else if((RttNumberNow == 3) || (RttNumberNow == 4))	/*针在排样位置*/
	...
	{		
		case 0:		/*吸样位置2（内圈）*/			
			if(RttNumberNow != 0)
			{
				...
				err = MCC_Move_PresetLocation(_gp_RTT,
												_gp_RTT->preset_location,		/*预设吸样位置2（内圈）*/
												5000,
												MCC_MOVE_DISTANCE,
												0,
												MCC_LINE_AUTO);
		...
		case 1:		/*吸样位置1（外圈）*/
			if(RttNumberNow != 1)
			{
				OSSemPost(_gp_TurnInhaleSemIn);	/*启动旋转吸10uL空气*/
				err = MCC_Move_PresetLocation(_gp_RTT,
												_gp_RTT->preset_location + 1,	/*预设吸样位置1（外圈）*/
												5000,
												MCC_MOVE_DISTANCE,
												0,
												MCC_LINE_AUTO);
		...
		case 2:		/*复位位置*/
			...
			{
//			err = MCC_Move_PresetLocation(_gp_RTT,
//											_gp_RTT->preset_location + 2,	/*预设复位位置*/
//											5000,
//											MCC_MOVE_DISTANCE,
//											0,
//											MCC_LINE_AUTO);
		...
		case 3:	/*R1排样位置*/			
			...
			{
				err = MCC_Move_PresetLocation(_gp_RTT,
												_gp_RTT->preset_location + 3,	/*预设R1排样位置*/
												5000,
												MCC_MOVE_DISTANCE,
												0,
												MCC_LINE_AUTO);
		...
		case 4:	/*R2排样位置*/
			...
			{
				err = MCC_Move_PresetLocation(_gp_RTT,
												_gp_RTT->preset_location + 4,	/*预设R2排样位置*/
												5000,
												MCC_MOVE_DISTANCE,
												0,
												MCC_LINE_AUTO);
...
bool Func_CmdNdlSample(void* p_buffer)
{
	...
	err = MCC_Move_Distance(_gp_INJ,
							...
							(double)p_data->size / 10.0 + _para_NEEDLE.multi_suction_multiply100/100.0,	/*单位uL,多吸5uL*/
							5000,
							MCC_MOVE_DISTANCE,
							0,
							MCC_LINE_AUTO);
...
bool Func_CmdNdlBlowoff(void* p_buffer)
{
	...
	err = MCC_Move_Distance(_gp_INJ,
							...
							(double)p_data->size / 10.0 + _para_NEEDLE.multi_displacement_multiply100/100.0,	/*单位uL，多排2.5uL*/
							5000,
							MCC_MOVE_DISTANCE,
							0,
							MCC_LINE_AUTO);
...
bool Func_CmdNdlWash(void* p_buffer)
{
	...
	switch(p_data->type)
	{
		case 0:	/*常规清洗*/	
			...
			Valve_Close(_gp_InnerWallCleaningValve);	/*关内壁清洗阀*/
			Valve_Close(_gp_OuterWallCleaningValve);	/*关外壁清洗阀*/
			OSTimeDlyHMSM(0, 0, 0, 200);  //加长延时，避免清洗液位还在高位时针运动出现的甩液
			break;
