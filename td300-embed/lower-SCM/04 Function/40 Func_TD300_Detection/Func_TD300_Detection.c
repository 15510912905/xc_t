...
#include "Func_TD300_Detection.h"

static DRV_MAX5402EUA_TYPE* _gp_Max5402eua[4] = {NULL};	/*定义数字电位器*/
...
//	Cmd_AddCmdNode(dlist, CMD_COM_RW_PARAMETER,		Func_Cmd_Com_RW_Parameter);			/*单个参数读写*/
	...
	Cmd_AddCmdNode(dlist, CMD_READ_ALL_AD,			Func_Cmd_ReadAllAd);
	return true;
}
...
/* 初始化 Func_TD300DetectionInit
  ...
  返回值：true，成功；false，失败
*/
bool Func_TD300DetectionInit(void)
{
	/*初始化增益*/
	_gp_Max5402eua[0] = Max5402euaInit(MAX5402EUA_1, 185, \
						_PORT_BUTT_, _PIN_BUTT_, \
						_PA_, _P7_, \
						_PA_, _P5_, \
						_PB_, _P10_);
	if(_gp_Max5402eua[0] == NULL)	/*定义数字电位器，编号1，端口PA7、PA5、PB10*/
	{
		return false;
	}
	...
	_gp_Ad7685 = Drv_Ad7685Init(AD7685_1, _SPI2_, _PB_, _P12_, _PB_, _P13_, _PB_, _P14_, _PB_, _P15_);	/*定义片外AD芯片，编号1，使用SPI2接口，端口PB12、PB13、PB14、PB15*/
	if(_gp_Ad7685 == NULL)
	{
		return false;
	}
	
	_gp_Sen = Drv_SwitchingSensorInit(SWITCHINGSENSOR_1, FALLING, 10, _PA_, _P3_);	/*定义采光光耦，编号1，下降沿响应，消抖延时10ms，端口PA3*/
	if(_gp_Sen == NULL)
	{
		return false;
	}
	...
	Max5402euaSet(_gp_Max5402eua[3], 144); //default 185
	Max5402euaSet(_gp_Max5402eua[2], 96); //default 185
	Max5402euaSet(_gp_Max5402eua[1], 64);  //default 185 
	Max5402euaSet(_gp_Max5402eua[0], 50); //default 185
...
bool Func_Cmd_ReadOneAd(void* p_buffer)
{
	...
	uint8_t i = 0, j = 0;
	uint32_t temp[10] = {0};	/*一个通道读10次，取平均*/
...
//				datas[i][j] = Ad7685OneAcq(_gp_Ad7685);
		for(i = 0; i< 4; i++)
		{	
			Max4634eubChoose(_gp_Max4634eub, i + 1);
			for(j = 0; j < 10; j++)								/*修改单通道采集次数为10次*/	
			{
...
//				temp[j] = datas[i][j];
//			}
			gp_Data->datas[0][i] = _Func_Average(temp, 10);		/*均值滤波*/
//		}
		}
		_Func_Sort4(&(gp_Data->datas[0][0]));					/*排序*/
		Drv_CANProtocol_SendDatasToBuffer(can_id, serial_number, cmd, frame_id, (uint8_t*)(gp_Data), 20, 44);	/*直接回复执行成功*/
	}
	else
	{
		gp_Data->count = 1;
...
//	uint32_t datas[4][10];							/*数据缓存*/	
	...
	gp_Data->count = p_data->count * 4;
//	Drv_SwitchingSensorSemSet(_gp_Sen, EN_SEM, FALLING);	/*开启IO下降沿信号量*/
	for(i = 0; i < p_data->count; i++)
	{
		timer = OSTICK_MS;
//		OSSemPend(_gp_Sen->sem, 5000, &err);/*等待光耦信号,超时时间1s*/
		while(1)
		{	
...
//					temp[j] = datas[k][j];
//				}
//				gp_Data->datas[i][k] = _Func_Average(temp, 10);		/*均值滤波*/
//			}
		timer = OSTICK_MS - timer;
		if(err != OS_ERR_NONE)	/*超时报错*/
			{
		...
		for(j = 0; j < 4; j++)
		{
			...
			for(k = 0; k < 10; k++)						/*修改单通道采集次数为10次*/			
			{
				temp[k] = Ad7685OneAcq(_gp_Ad7685);					/*读取AD值*/
			}
			gp_Data->datas[i][j] = _Func_Average(temp, 10);			/*均值滤波*/
		}
	...
	len = p_data->count * 4 * 4 + 4;	/*计算待发送数据总长度*/
...
bool Func_Cmd_SetGain(void* p_buffer)
{
	...
	Max5402euaSet(_gp_Max5402eua[_Func_Mapping4(p_data->channel - 1)], p_data->gain);
...
static bool Func_Cmd_ReadAllAd(void* p_buffer)
{
	...
	while(i < 2001)
	{
		...
		if((last_op == 1) && (_gp_AllAD[i].op == 0))	/*下降沿*/
		{
			...
			for(j = 0; j < 4; j++)	/*模拟正常测试*/
			{
				Max4634eubChoose(_gp_Max4634eub,  1);	/*通道选择*/
				for(k = 0; k < 10; k++)
				{
					temp[k] = Ad7685OneAcq(_gp_Ad7685);	/*读取AD值*/
				}
				gp_Data->datas[0][j] = _Func_Average(temp, 10);	/*滤波*/
			}
//			_Func_Sort4(&(gp_Data->datas[0][0]));	/*排序*/
			for(j = 0; j < 3; j++)
			{
				_gp_AllAD[i].op = 2;
				_gp_AllAD[i++].ad = gp_Data->datas[0][j];
			}
			_gp_AllAD[i].op = 2;
			_gp_AllAD[i].ad = gp_Data->datas[0][j];
		}
