...
typedef struct
{
	uint32_t start_fre;				/*启动频率*/
	uint32_t end_fre;				/*最高频率*/
	uint32_t ech_lader_step;		/*每台阶微步*/
	uint32_t up_max_lader;			/*最大加速台阶*/
	uint32_t s_par;					/*S型曲线的形状参数*/
...
typedef struct
{
    ...
    uint32_t  slowFre;					/*慢速运行频率*/
    uint32_t* p_fre_sp;					/*电机加速频率表*/
    uint32_t* p_fre_sd;					/*电机减速频率表*/
	...
	uint32_t* line_hig_fre;				/*高速运动频率表*/
	uint32_t* line_low_fre;				/*低速运动频率表*/
...
/* SP运动启动函数
  ...
  返回值：无
*/
extern void Motor_MoveSP_Begin(DRV_STEPMOTOR_TYPE* p_unit, uint8_t dir, uint32_t step, uint32_t slowstep, bool line_switch);
...
/*
  ...
  返回值：无
*/
extern void CalcMotorFre(uint8_t subdivision, DRV_STEPMOTOR_LINE_TYPE* moto_line, uint32_t* fre_arry);
