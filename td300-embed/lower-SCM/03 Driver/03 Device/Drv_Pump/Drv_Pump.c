...
{	
	...
	BSP_WritePin(p_unit->signal_port_number, p_unit->signal_pin_number, 1);
...
{
	_PumpPowerUp(p_unit);
	BSP_SetTimer(p_unit->timer_number, 8000000 * 2 / p_unit->fre);	/*定时器中断时基8MHz，初始化定时器中断平率*/
	BSP_EnTimer(p_unit->timer_number);				/*启动定时器*/
	BSP_WritePin(p_unit->signal_port_number, p_unit->signal_pin_number, 0);
...
void PumpStop(DRV_PUMP_TYPE* p_unit)
{
	...
	BSP_DisTimer(p_unit->timer_number);				/*关闭定时器*/
	BSP_WritePin(p_unit->signal_port_number, p_unit->signal_pin_number, 1);
	p_unit->status = PUMP_STOP;
}
		...
		if(PumpIsStart(_gp_PumpUnits[i]))	/*泵启动*/
		{
			switch(_gp_PumpUnits[i]->type)
			{
				...
				case PUMP_ADJUSTABLE_SPEED_WITH_SIGNAL: /*信号调速泵*/
					_gp_PumpUnits[i]->duty_cycle_count++;	/*占空比计数*/
					if(_gp_PumpUnits[i]->duty_cycle_count <= _gp_PumpUnits[i]->duty_cycle/5)
					{
 						_PumpSignalOff(_gp_PumpUnits[i]);	/*信号拉低*/
						
					}
					else if(_gp_PumpUnits[i]->duty_cycle_count <= 20)
					{
						_PumpSignalUp(_gp_PumpUnits[i]);	/*信号拉高*/
						if(_gp_PumpUnits[i]->duty_cycle_count == 20 )
						_gp_PumpUnits[i]->duty_cycle_count = 0;	
					}
					break;
